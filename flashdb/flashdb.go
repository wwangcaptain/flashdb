...
import (
	"errors"
	"flashdb/internal/append_only_log"
...
type (
	FlashDB struct {
		...
		evictors []EvictionHandler // background manager to delete keys periodically
	}
)
...
func New(config *Config) (*FlashDB, error) {
	...
	if db.persist {
		opts := append_only_log.DefaultOptions
		opts.NoSync = config.NoSync

		l, err := append_only_log.Open(config.Path, opts)
		if err != nil {
			return nil, err
		}
